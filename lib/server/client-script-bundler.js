...
var readAudioClips = function(audioFolder) {
  var audioClips = {};
  fs.readdirSync(audioFolder).forEach(function(audioFile) {
    audioClips[audioFile.split('.')[0]] = '/audio/' + audioFile;
  });
  return audioClips;
};
...
exports.compileScriptBundle = function(options, callback) {
  ...
  var audioClips = readAudioClips(options.audioFolder);
  ...
  browserify(path.join(__dirname, '../client/index.js')).bundle({
    insertGlobalVars: {
      ...
      audioClips: function() { return JSON.stringify(audioClips); }
    }
  }, callback);
};
