var ko = require('knockout'),
    ...
    audioClips = require('./globals.js').audioClips,
  ...
  return zip(card.hanzi.split(''), card.pinyin.split(' ')).map(function(zipped) {
    ...
    return {
      ...
      clickHandler: throttle(function() {
        var audioClipPinyinKey = pinyin;
        if (!pinyinParts[2]) {
          audioClipPinyinKey += '1';
        }
        if (audioClips[audioClipPinyinKey]) {
          new Audio(audioClips[audioClipPinyinKey]).play();
        }
      }, 1000),
      pinyin: {
        text: pinyinParts[1],
